...
# . Call simulation ----
  res <- sim_pop(
    nyears = 50,
    river = 'Saco',#get_rivers()[sample(1:length(get_rivers()), 1)],
    max_age = NULL,
    nM = NULL,
    fM = 0,
    ...
    type= 'functional',#sample(c('total','functional'), 1),
    output='last'
    )
...
inds <- grep("spawners_", names(resdf))

spawners <- rowSums(resdf[ , inds])
hist(spawners)
