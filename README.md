...
![**Figure 1**: Graphical overview of the `anadrofish` workflow (right) compared to conceptual life-history model for anadromous fishes (left). Helper functions are used within the `sim_pop()` function along with built-in data sets to simulate anadromous fish populations.](https://github.com/danStich/anadrofish/blob/master/inst/images/conceptual-model.jpg?raw=true)
...
![Figure 2: Example result from Sebasticook River simulation](https://github.com/danStich/anadrofish/blob/master/inst/images/seb_ale.jpg?raw=true)
